...
class ResultController extends Controller
{
    public function index(Request $request)
    {
        ...
        $centers = PrimaryResult::select('exam_centre')->distinct()->orderBy('exam_centre', 'asc')->get();

        if ($request->center) {
            $query->where('exam_centre', $request->center);
        }

        if ($request->admin_exam_id) {
            $query->where('admin_result_id', $request->admin_exam_id);
        }

        // $primaryresults = $query->get();
        $primaryresults = $query->orderBy('std', 'asc')->get();
        return view('admin.result.index', compact('primaryresults', 'centers'));
    }
...
public function toggleMarksheetButton(Request $request)
{
    ...
    public function meritList(Request $request)
    {
        ...
        $states = PrimaryResult::select('state')->whereNotNull('state')->distinct()->orderBy('state', 'asc')->get();

        // Get unique divisions for dropdown
        $divisions = PrimaryResult::select('division')->whereNotNull('division')->distinct()->orderBy('division', 'asc')->get();

        // Get unique districts for dropdown
        $districts = PrimaryResult::select('district')->whereNotNull('district')->distinct()->orderBy('district', 'asc')->get();

        // Get unique talukas for dropdown
        $talukas = PrimaryResult::select('taluka')->whereNotNull('taluka')->distinct()->orderBy('taluka', 'asc')->get();
        ...
        return view('admin.result.merit_list', compact(
            'meritStudents',
            'centers',
            'states',
            'divisions',
    ...
    public function downloadCenterCertificates($center)
    {
        $results = PrimaryResult::where('exam_centre', $center)->get();

        // return view('admin.result.bulk_certificates', compact('results'));
        $pdf = Pdf::loadView('admin.result.bulk_certificates', compact('results'));
        ...
        try {
            ...
            foreach ($states as $state) {
                ...
                foreach ($stateStudents as $student) {
                    ...
                    $rank =  MeritLIstRank::where('role','state')->first();
            ...
            foreach ($divisions as $division) {
                ...
                foreach ($divisionStudents as $student) {
                    ...
                    $rank =  MeritLIstRank::where('role','division')->first();
                    if ($currentRank <= ($rank->rank ?? 0)) {
            ...
            foreach ($districts as $district) {
                ...
                foreach ($districtStudents as $student) {
                    ...
                    $rank =  MeritLIstRank::where('role','district')->first();
                    if ($currentRank <= ($rank->rank ?? 0)) {
            ...
            foreach ($talukas as $taluka) {
                ...
                foreach ($talukaStudents as $student) {
                    ...
                    // Only assign merit if within top 3 ranks
                    $rank =  MeritLIstRank::where('role','taluka')->first();
                    if ($currentRank <= ($rank->rank ?? 0)) {
            ...
            foreach ($centers as $center) {
                ...
                foreach ($centerStudents as $student) {
                    ...
                    // Only assign merit if within top 3 ranks
                    $rank =  MeritLIstRank::where('role','center')->first();
                    if ($currentRank <= ($rank->rank ?? 0)) {
    ...
    public function downloadMeritListPdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('admin.result.merit_list_pdf', compact('meritStudents'));
    ...
    public function show($id)
    {
        ...
        return view('admin.result.show', compact('primaryresult'));
    }

    public function edit($id)
    {
        $primaryresult = PrimaryResult::findOrFail($id);
        return view('admin.result.edit', compact('primaryresult'));
    }
    ...
    {
        $adminExamId = $request->input('admin_exam_id');
         
        if ($adminExamId) {
            PrimaryResult::where('admin_result_id', $adminExamId)->delete();
        } else {
