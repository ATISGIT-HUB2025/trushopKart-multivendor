...
class SubDistrictHeadController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['monthlyEarning'] = SchoolForVisiting::where('created_by', $userId)
                        // ->whereBetween('created_at', [$startOfMonth, $endOfMonth])
                        ->sum('total_bill');


        $data['talukaTodaysOrders'] = Order::where(function ($query) {
        ...
        $data['talukaTotalOrders'] = Order::where(function ($query) {
            $query->whereHas('user', function ($q) {
                $q->where('role', 'taluka_head');
            });
        })->count();

        $data['talukaTotalOrderspending'] = Order::where('order_status', 'pending')->where(function ($query) {
            $query->whereHas('user', function ($q) {
                $q->where('role', 'taluka_head');
            });
        })->count();

        $data['talukaTotalAdmissions'] = SchoolForVisiting::where(function ($query) {
            $query->whereHas('user', function ($q) {
                $q->where('role', 'taluka_head');
            });
        })->count();

        $data['talukaMonthlyEarning'] = SchoolForVisiting::where(function ($query) {
            $query->whereHas('user', function ($q) {
                $q->where('role', 'taluka_head');
            });
        })->sum('total_bill');
        ...
        $data['BulkAdmission_count'] =  StudentAdmission::where('teacher_id', auth()->id())->count();
           // return $data;
        return view('subdistricthead.dashboard.index', $data);
    }
    ...
    { 
        ...
        return view('subdistricthead.school_for_visiting.index', $data);
    }
    
    public function schoolForVisitingCreate()
    {
       $data['standards'] = Standard::limit(10)->get();
        return view('subdistricthead.school_for_visiting.create', $data);
    }
    ...
    public function schoolForVisitingStore(Request $request)
    {
        ...
        return redirect()->route('subdistrict.school-for-visiting.index')->with('success', 'visit added successfully.');
    }
    ...
    public function schoolForVisitingExcel(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('subdistrict.school-for-visiting.index');
        } catch (\Exception $e) {
    ...
    public function schoolForVisitingEdit($id)
{
    ...
    return view('subdistricthead.school_for_visiting.edit', $data);
}
...
public function schoolForVisitingUpdate($id,Request $request)
{
    ...
    return redirect()->route('subdistrict.school-for-visiting.index')->with('success', 'visit Update successfully.');
}

public function schoolForVisitingDestroy($id)
{
    SchoolForVisiting::where('id',$id)->delete();
    return redirect()->route('subdistrict.school-for-visiting.index')->with('success', 'visit Delete successfully.');
}
