    ...
    transform: translateX(24px);
            ...
            <button type="button" id="deactivateAll" class="btn btn-danger mb-3">All Inactive</button>
                ...
                <table class="table table-bordered" id="resultsTable">
                    ...
                    <tbody>
                        @foreach($centers as $key => $result)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td>{{ $result->exam_centre }}</td>
                            <td>
            <label class="switch">
                <input type="checkbox" class="toggle-status" data-id="{{ $result->exam_centre }}"
                    {{ $result->exam_center_status ? 'checked' : '' }}>
...
$(document).ready(function () {
    var table = $('#resultsTable').DataTable({
        "processing": true,
        "serverSide": false,
        "destroy": true, // Ensures DataTable reinitializes properly
    ...
    function handleStatusChange() {
        ...
        $.ajax({
            url: "{{ route('admin.exam.center.status-change') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    // Run function when DataTables redraws the table
    // table.on('draw', function () {
    //     console.log("new23")
    //     handleStatusChange();
    // });

    $('#resultsTable').on('page.dt', function () {
        $('#resultsTable').DataTable().draw(false); // Redraws the table while keeping the current page
    });
...
<script>
    ...
    $('#resultsTable').DataTable({
        ...
        // Bind the event on the DataTable instance
        table.on('page.dt', function () {
            console.log("Pagination changed, redrawing table");
    ...
    $.ajax({
        url: "{{ route('admin.exam.center.status-change') }}", // Update this with your route
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            examCenter: examCenter,
...
$(document).ready(function () {
        ...
        function updateStatusForAll(status) {
            ...
            $.ajax({
                url: "{{ route('admin.exam.center.status-change') }}", // Change this to your actual endpoint
                method: 'POST',
                data: {
                    ids: ids,
                    status: status,
