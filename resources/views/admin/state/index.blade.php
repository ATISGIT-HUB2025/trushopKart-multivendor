    ...
    transform: translateX(24px);
            ...
            <button type="button" id="deactivateAll" class="btn btn-danger mb-3">All Inactive</button>
                ...
                <table class="table table-bordered" id="resultsTable">
                    ...
                    <tbody>
                        @foreach($states as $key => $result)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td>{{ $result->state }}</td>
                            <td>
            <label class="switch">
                <input type="checkbox" class="toggle-status" data-id="{{ $result->state }}"
                    {{ $result->state_status ? 'checked' : '' }}>
...
<script>
    ...
    $.ajax({
        url: "{{ route('admin.state.status-change') }}", // Update this with your route
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
...
$(document).ready(function () {
        ...
        function updateStatusForAll(status) {
            ...
            $.ajax({
                url: "{{ route('admin.state.status-change') }}", // Change this to your actual endpoint
                method: 'POST',
                data: {
                    ids: ids,
                    status: status,
