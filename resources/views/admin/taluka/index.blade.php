...
<div class="card-body">
    <!-- Add buttons for activating and deactivating all -->
    ...
    <table class="table table-bordered" id="resultsTable">
        ...
        <tbody>
            @foreach($talukas as $key => $result)
            <tr>
                <td>{{ $key+1 }}</td>
                <td>{{ $result->taluka }}</td>
                <td>
                    <label class="switch">
                        <input type="checkbox" class="toggle-status" data-id="{{ $result->taluka }}"
                            {{ $result->taluka_status ? 'checked' : '' }}>
...
<script>
    ...
    $.ajax({
        url: "{{ route('admin.taluka.status-change') }}", // Update this with your route
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
...
$(document).ready(function () {
        ...
        function updateStatusForAll(status) {
            ...
            $.ajax({
                url: "{{ route('admin.taluka.status-change') }}", // Change this to your actual endpoint
                method: 'POST',
                data: {
                    ids: ids,
                    status: status,
