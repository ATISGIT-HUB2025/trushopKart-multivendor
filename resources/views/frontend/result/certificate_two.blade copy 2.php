...
<body>
    ...
    function getBase64Image($imagePath) {
        ...
        return ''; 
            ...
            $maxLength = 39; 
                $heading = $masterInfo->heading ?? '';
                if (strlen($heading) > $maxLength) {
                    $heading = wordwrap($heading, $maxLength, "<br />", true);
                }

            @endphp


            <img src="{{ $base64Logo }}"  alt="Award Icon" />
        ...
        <h2 style="margin-bottom: 25px">    {!! $heading !!}</h2>
    ...
    <!-- Replace the existing table rows with this code -->
<div class="table-container">
    <table>
        ...
        @php
            // Define max marks based on standard
            $paperOneMaxMarks = 100; // Default
            $paperTwoMaxMarks = 100; // Default
            $hasSecondPaper = true;
            
            // Set max marks based on standard (std)
            switch($primaryResult->std) {
                case '1':
                case '2':
                    $paperOneMaxMarks = 150;
                    $hasSecondPaper = false;
                    break;
                case '3':
                    $paperOneMaxMarks = 100;
                    $paperTwoMaxMarks = 100;
                    break;
                case '4':
                case '5':
                case '6':
                ...
                case '8':
                    $paperOneMaxMarks = 150;
                    $paperTwoMaxMarks = 150;
                    break;
            }
            
            // Calculate total max marks
            $totalMaxMarks = $hasSecondPaper ? ($paperOneMaxMarks + $paperTwoMaxMarks) : $paperOneMaxMarks;
        @endphp
<?php

$stateRank = PrimaryResult::where('state', $primaryResult->state)->where('std', $primaryResult->std)->where('medium', $primaryResult->medium)->where('percentage', '>', $primaryResult->percentage)->count() + 1;
        
$divisionRank = PrimaryResult::where('state', $primaryResult->state)->where('division', $primaryResult->division)->where('std', $primaryResult->std)->where('medium', $primaryResult->medium)
    ->where('percentage', '>', $primaryResult->percentage)
    ->count() + 1;

$districtRank = PrimaryResult::where('state', $primaryResult->state)->where('division', $primaryResult->division)->where('district', $primaryResult->district)->where('std', $primaryResult->std)->where('medium', $primaryResult->medium)
    ->where('percentage', '>', $primaryResult->percentage)
    ->count() + 1;

$talukaRank = PrimaryResult::where('state', $primaryResult->state)->where('division', $primaryResult->division)->where('district', $primaryResult->district)->where('taluka', $primaryResult->taluka)->where('std', $primaryResult->std)->where('medium', $primaryResult->medium)
    ->where('percentage', '>', $primaryResult->percentage)
    ->count() + 1;

$centerRank = PrimaryResult::where('state', $primaryResult->state)->where('division', $primaryResult->division)->where('district', $primaryResult->district)->where('taluka', $primaryResult->taluka)->where('exam_centre', $primaryResult->exam_centre)->where('std', $primaryResult->std)->where('medium', $primaryResult->medium)
    ->where('percentage', '>', $primaryResult->percentage)
    ->count() + 1;
...
?>
        <tr>
            <td>Paper 1</td>
            <td>{{ $paperOneMaxMarks }}</td>
            <td>{{ $primaryResult->first_paper }}</td>
            <td rowspan="{{ $hasSecondPaper ? 4 : 3 }}">{{ $primaryResult->percentage }}%</td>
            <th>STATE</th>
        ...
        <tr>
            <td colspan="{{ $hasSecondPaper ? 4 : 4 }}"></td>
            <th>DISTRICT</th>
            <td>{{ $primaryResult->district_merit }}</td>
        </tr>
        ...
        <tr>
            <td colspan="{{ $hasSecondPaper ? 4 : 4 }}"></td>
            <th>TALUKA</th>
            <td>{{ $primaryResult->taluka_merit }}</td>
        </tr>
    </table>
</div>
        ...
        <div style="width: 100%; text-align: center; margin-top: 20px;">
            ...
            <div style="display: inline-block; width: 25%; margin-top:20px; text-align: center; vertical-align: top; margin: 0 60px;">
                ...
                @if($base64AdminSig)
                <img src="{{ $base64AdminSig }}" alt="Administrator Signature" style="width: 120px;" />
              @endif
                </div>
                <div style="width: 100%; border-top: 2px solid black; padding-top: 5px; font-size: 14px;">
                  
               {{$masterInfo->administrator_name ?? ''}}<br />(ADMINISTRATOR)
                </div>
            </div>
            <!-- Second Group -->
            <div style="display: inline-block; width: 25%; text-align: center; vertical-align: top; margin: 0 80px;">
              ...
              @endif
                ...
                <div style="width: 100%; border-top: 2px solid black; padding-top: 5px; font-size: 14px;">
                {{$masterInfo->director_name ?? ''}}<br />(DIRECTOR)
                </div>
            </div>
        </div>
